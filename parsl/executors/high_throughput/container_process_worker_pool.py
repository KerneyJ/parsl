...
class Manager:
    ...
    def __init__(self,
        ...
        logger.info("Determine which start method to use {}".format(start_method))
        if start_method == "fork":
            ...
            self.result_addr = "tcp://{}:{}".format(self.container_ip, self.base_port + self.incr_port)
            self.incr_port += 1
            self.result_server.bind(self.result_addr)
        else:
            ...
            if self.task_incoming in socks and socks[self.task_incoming] == zmq.POLLIN:
                ...
                if tasks == 'STOP':
                    ...
                    for task in tasks:
                        ...
                        logger.info("Sending task to worker {}".format(worker_sock))
                        worker_sock.send(pickle.dumps(task))
                        ack = worker_sock.recv()
    ...
    def start(self):
        ...
        for worker_id in range(self.worker_count):
            ...
            #    p = subprocess.Popen(["docker", "run", "-P", "--rm", self.container_path]) # implicitly assumes that container_path is user:tag
                ...
                p = subprocess.Popen(["singularity", "exec", self.container_img_path, "python3", "/opt/worker.py", self.container_ip, str(self.base_port + self.incr_]ort]))
            else:
                pass
            ...
            worker_sock.send(self.result_addr.encode())
...
if __name__ == "__main__":
    ...
    parser.add_argument("--start-method", type=str, choices=["container"], default="fork",
                        help="Method used to start new worker processes")
    parser.add_argument("--container_type", type=str, default=None,
                        help="type of container, singularity or docker")
    parser.add_argument("--container_img_path", type=str, default=None,
                        help="path of the container being used")
    parser.add_argument("--container_ip", type=str, default=None,
                        help="ip that the manager should use")
    ...
    try:
        ...
        logger.info("Container Type: {}".format(args.container_type))
        logger.info("Container Image Path: {}".format(args.container_img_path))
        logger.info("Container ip: {}".format(args.container_ip))
        ...
        manager = Manager(task_port=args.task_port,
                          ...
                          container_img_path=args.container_img_path,
