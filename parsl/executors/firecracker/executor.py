...
class FirecrackerExecutor(BlockProviderExecutor, RepresentationMixin):
    """Executor designed for cluster-scale

    The FirecrackerExecutor system has the following components:
      1. The FirecrackerExecutor instance which is run as part of the Parsl script.
      2. The Interchange which acts as a load-balancing proxy between workers and Parsl
      3. The multiprocessing based worker pool which coordinates task execution over several
         cores on a node.
      4. ZeroMQ pipes connect the FirecrackerExecutor, Interchange and the process_worker_pool
    ...
    def __init__(self,
        ...
        logging.disable()
        logger.debug("Initializing FirecrackerExecutor")
    ...
    def initialize_scaling(self):
        ...
        logger.debug("Starting FirecrackerExecutor with provider:\n%s", self.provider)
    ...
    def _start_queue_management_thread(self):
        ...
        if self._queue_management_thread is None:
            ...
            self._queue_management_thread = threading.Thread(target=self._queue_management_worker, name="FC-Queue-Management-Thread")
    ...
    def submit(self, func, resource_specification, *args, **kwargs):
        ...
        if resource_specification:
            logger.error("Ignoring the resource specification. "
                         "Parsl resource specification is not supported in Firecracker Executor. "
                         "Please check WorkQueueExecutor if resource specification is needed.")
            raise UnsupportedFeatureError('resource specification', 'Firecracker Executor', 'WorkQueue Executor')
    ...
    def shutdown(self):
        ...
        logger.info("Attempting FirecrackerExecutor shutdown")
        self.interchange_proc.terminate()
        logger.info("Finished FirecrackerExecutor shutdown attempt")
